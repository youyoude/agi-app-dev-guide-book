...
> "执行环境是AGI应用的舞台，不同的任务需要不同的舞台。智能环境管理的艺术在于为每个演出提供最合适的舞台，让智能应用的每一次执行都能发挥最佳性能。"
...
基于我们修正的架构关系，工具扩展运行层会向智能环境层申请不同类型的执行环境。AGI应用的环境需求具有以下特点：
...
容器化环境是AGI应用最常用的执行环境，需要完善的生命周期管理：
...
对于大规模AGI应用部署，Kubernetes提供了强大的容器编排能力：
...
**下一步学习**：掌握了执行环境管理后，我们将在4.4.3节学习智能资源调度与管理，了解如何实现AGI应用的智能化资源分配和动态伸缩。
