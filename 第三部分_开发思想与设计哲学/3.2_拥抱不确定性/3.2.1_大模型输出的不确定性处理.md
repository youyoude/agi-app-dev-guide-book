...
建立全面的监控体系来跟踪大模型调用的健康状况：

```java
public class LLMMetricsCollector {
    
    public void recordLLMCall(String model, long duration, boolean success, String errorType) {
        // 记录调用时长
        Metrics.timer("llm.call.duration", "model", model).record(duration, TimeUnit.MILLISECONDS);
        
        // 记录成功率
        Metrics.counter("llm.call.total", "model", model, "success", String.valueOf(success)).increment();
        
        // 记录错误类型
        if (!success && errorType != null) {
            Metrics.counter("llm.error", "model", model, "error_type", errorType).increment();
        }
    }
}
```
...
// 异常情况的详细记录
if (choices == null || choices.isEmpty()) {
    log.error("{} Invalid response structure: {}", context.getRequestId(), response);
    // 记录到异常分析系统
    ExceptionAnalyzer.recordPattern("empty_choices", response, context);
}
```
