...
public class ConsensusManager {
    ...
    public ConsensusResult reachConsensus(String proposalId, Object proposal, List<String> participants) {
        // 第一阶段：准备阶段
        Map<String, PrepareResponse> prepareResponses = sendPrepareRequests(proposalId, participants);
        
        // 检查是否达到法定人数
        if (prepareResponses.size() < getQuorumSize(participants.size())) {
            return ConsensusResult.failure("Insufficient prepare responses");
        }
        
        // 第二阶段：接受阶段
        Map<String, AcceptResponse> acceptResponses = sendAcceptRequests(proposalId, proposal, participants);
        
        // 检查接受情况
        long acceptCount = acceptResponses.values().stream()
            .mapToLong(r -> r.isAccepted() ? 1 : 0)
            .sum();
            
        if (acceptCount >= getQuorumSize(participants.size())) {
            sendCommitRequests(proposalId, proposal, participants);
            return ConsensusResult.success(proposal);
        } else {
            return ConsensusResult.failure("Insufficient accepts");
        }
    }
    
    private int getQuorumSize(int totalParticipants) {
        return (totalParticipants / 2) + 1; // 简单多数
    }
}
```
...
public class ConsistencyChecker {
    ...
    public ConsistencyReport checkGlobalConsistency(List<String> agentIds) {
        ConsistencyReport report = new ConsistencyReport();
        
        // 收集所有智能体的状态快照
        Map<String, StateSnapshot> snapshots = collectStateSnapshots(agentIds);
        
        // 检查内存一致性
        MemoryConsistencyResult memoryResult = checkMemoryConsistency(snapshots);
        report.setMemoryConsistency(memoryResult);
        
        // 检查任务状态一致性
        TaskConsistencyResult taskResult = checkTaskConsistency(snapshots);
        report.setTaskConsistency(taskResult);
