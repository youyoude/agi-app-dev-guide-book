...
public class QualityMonitor {
    ...
    public QualityScore evaluateStepQuality(String stepResult, AgentContext context) {
        QualityScore score = new QualityScore();
        
        // 内容质量评估
        score.setContentQuality(evaluateContentQuality(stepResult));
        
        // 进度贡献度评估
        score.setProgressContribution(evaluateProgress(stepResult, context));
        
        // 资源效率评估
        score.setResourceEfficiency(evaluateResourceUsage(context));
        
        return score;
    }
    
    public boolean shouldInterrupt(QualityScore score) {
        // 如果连续几步质量都很低，建议终止
        return score.getOverallScore() < 0.3;
    }
}
```
...
public class DynamicAdjustmentEngine {
    ...
    public void adjustExecutionStrategy(QualityScore currentScore, AgentContext context) {
        if (currentScore.getContentQuality() < 0.5) {
            // 降低temperature，提高输出稳定性
            context.getLlmSettings().setTemperature(0.1);
        }
        
        if (currentScore.getProgressContribution() < 0.4) {
            // 增加系统提示，引导更好的推理
            String enhancedPrompt = enhanceSystemPrompt(context.getSystemPrompt());
            context.setSystemPrompt(enhancedPrompt);
        }
    }
}
```
