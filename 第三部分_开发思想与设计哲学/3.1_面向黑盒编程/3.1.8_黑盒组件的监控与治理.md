...
public class AIMetricsCollector {
    ...
    public void recordLLMCall(String provider, String model, int inputTokens, int outputTokens, long duration) {
        // 调用次数
        Counter.builder("ai.llm.calls.total")
        ...
        // Token使用量
        Counter.builder("ai.llm.tokens.total")
            .tag("provider", provider)
            .tag("model", model)
            .tag("type", "input")
            .register(meterRegistry)
    ...
    public void recordToolExecution(String toolName, long duration, boolean success, String errorType) {
        Timer.builder("ai.tool.execution.time")
            .tag("tool", toolName)
            .tag("success", String.valueOf(success))
            .register(meterRegistry)
            .record(duration, TimeUnit.MILLISECONDS);
        ...
        if (!success && errorType != null) {
            Counter.builder("ai.tool.errors.total")
                .tag("tool", toolName)
    ...
    public void recordBusinessMetrics(String requestType, int userCount, double satisfaction) {
        ...
        // 用户满意度
        Gauge.builder("ai.user.satisfaction")
            .tag("type", requestType)
...
public class PerformanceMonitoringAspect {
     ...
     * 监控工具调用
     */
    @Around("execution(* com.ai.tools.*.execute(..))")
...
public class AIAlertEngine {
        ...
        for (AlertRule rule : alertRules) {
            try {
                if (rule.shouldAlert()) {
                    ...
                    // 记录告警指标
                    Counter.builder("ai.alerts.triggered")
    ...
    private double getLLMErrorRate() {
        // 从metrics中计算LLM错误率
        return calculateErrorRate("ai.llm.calls.total", "ai.llm.errors.total");
    }
    
    private double getAgentTimeoutRate() {
        // 计算智能体超时率
        return calculateTimeoutRate("ai.agent.execution.time");
    }
    ...
    private double getUserSatisfaction() {
        // 计算用户满意度
        return calculateAverageGaugeValue("ai.user.satisfaction");
    }
}
