...
从上图可以看出，流式通信贯穿整个系统的各个层次：前端通过SSE接收后端推送的实时消息，后端作为调度中枢协调各类工具服务的调用，工具服务层通过流式协议与外部AI模型和MCP服务进行交互。这种层层嵌套的流式通信架构，确保了用户能够实时感知到AI Agent的思考过程和执行进度。接下来，我们将逐一剖析各个服务的技术实现细节和通信机制。
