# Summary

## 序言部分

* [自序](自序.md)
* [作者简介](作者简介.md)
* [术语表](术语表.md)

## 前言部分

* [引言：AI时代](前言部分/引言：AI时代.md)
* [Agentic AI：通向AGI应用的关键前站与智能涌现之路](前言部分/Agentic_AI：通向AGI应用的关键前站与智能涌现之路.md)
* [打造辅助者不要打造替代者](前言部分/打造辅助者不要打造替代者.md)
* [软件工程师的新机遇](前言部分/软件工程师的新机遇.md)
* [如何使用本书](前言部分/如何使用本书.md)

## 第零部分：Agentic AI应用的基础认知

* [0.1 什么是Agentic AI？](第零部分_Agentic_AI应用的基础认知/0.1_什么是Agentic_AI.md)
* [0.2 为什么需要新的开发理念？](第零部分_Agentic_AI应用的基础认知/0.2_为什么需要新的开发理念.md)
* [0.3 如何构建Agentic AI应用系统？](第零部分_Agentic_AI应用的基础认知/0.3_如何构建Agentic_AI应用系统.md)
* [0.4 深入理解：理论基础与前沿发展](第零部分_Agentic_AI应用的基础认知/0.4_深入理解_理论基础与前沿发展.md)

## 第一部分：背景与问题

### 1.1 Agentic AI应用与传统应用的核心差异

* [1.1.0 章节导引：理解Agentic AI应用中的不确定性](第一部分：背景与问题/1.1_Agentic_AI应用与传统应用的核心差异/1.1.0_章节导引：理解Agentic_AI应用中的不确定性.md)
* [1.1.1 大语言模型的固有不确定性](第一部分：背景与问题/1.1_Agentic_AI应用与传统应用的核心差异/1.1.1_大语言模型的固有不确定性.md)
* [1.1.2 任务复杂度引发的不确定性](第一部分：背景与问题/1.1_Agentic_AI应用与传统应用的核心差异/1.1.2_任务复杂度引发的不确定性.md)
* [1.1.3 多智能体协作的不确定性](第一部分：背景与问题/1.1_Agentic_AI应用与传统应用的核心差异/1.1.3_多智能体协作的不确定性.md)
* [1.1.4 用户需求的不确定性](第一部分：背景与问题/1.1_Agentic_AI应用与传统应用的核心差异/1.1.4_用户需求的不确定性.md)

### 1.2 从传统开发到Agentic AI开发的转变挑战

* [1.2.0 章节导引：Agentic AI开发的七大核心转变](第一部分：背景与问题/1.2_从传统开发到Agentic_AI开发的转变挑战/1.2.0_章节导引：Agentic_AI开发的七大核心转变.md)
* [1.2.1 开发范式的转变](第一部分：背景与问题/1.2_从传统开发到Agentic_AI开发的转变挑战/1.2.1_开发范式的转变.md)
* [1.2.2 系统架构的差异](第一部分：背景与问题/1.2_从传统开发到Agentic_AI开发的转变挑战/1.2.2_系统架构的差异.md)
* [1.2.3 错误处理机制的变化](第一部分：背景与问题/1.2_从传统开发到Agentic_AI开发的转变挑战/1.2.3_错误处理机制的变化.md)
* [1.2.4 测试策略的革新](第一部分：背景与问题/1.2_从传统开发到Agentic_AI开发的转变挑战/1.2.4_测试策略的革新.md)
* [1.2.5 性能评估的重新定义](第一部分：背景与问题/1.2_从传统开发到Agentic_AI开发的转变挑战/1.2.5_性能评估的重新定义.md)
* [1.2.6 部署和运维的复杂性](第一部分：背景与问题/1.2_从传统开发到Agentic_AI开发的转变挑战/1.2.6_部署和运维的复杂性.md)
* [1.2.7 开发团队技能要求的变化](第一部分：背景与问题/1.2_从传统开发到Agentic_AI开发的转变挑战/1.2.7_开发团队技能要求的变化.md)

## 第二部分：Agentic AI应用的开发基础

### 2.1 流式通信

* [2.1.1 流式通信基础与应用场景](第二部分_Agentic_AI应用的开发基础/2.1_流式通信/2.1.1_流式通信基础与应用场景.md)
* [2.1.2 服务端流式响应架构设计](第二部分_Agentic_AI应用的开发基础/2.1_流式通信/2.1.2_服务端流式响应架构设计.md)
* [2.1.3 客户端流式数据接收处理](第二部分_Agentic_AI应用的开发基础/2.1_流式通信/2.1.3_客户端流式数据接收处理.md)
* [2.1.4 流式通信的可靠性保障](第二部分_Agentic_AI应用的开发基础/2.1_流式通信/2.1.4_流式通信的可靠性保障.md)
* [2.1.5 AI-Agent中的流式交互模式](第二部分_Agentic_AI应用的开发基础/2.1_流式通信/2.1.5_AI-Agent中的流式交互模式.md)
* [2.1.6 性能优化与监控](第二部分_Agentic_AI应用的开发基础/2.1_流式通信/2.1.6_性能优化与监控.md)
* [2.1.7 企业级部署与运维](第二部分_Agentic_AI应用的开发基础/2.1_流式通信/2.1.7_企业级部署与运维.md)

### 2.2 异步并发编程

* [2.2.1 Agentic AI应用中的异步编程基础](第二部分_Agentic_AI应用的开发基础/2.2_异步并发编程/2.2.1_AI应用中的异步编程基础.md)
* [2.2.2 多语言异步编程实践](第二部分_Agentic_AI应用的开发基础/2.2_异步并发编程/2.2.2_多语言异步编程实践.md)
* [2.2.3 流式响应与实时通信](第二部分_Agentic_AI应用的开发基础/2.2_异步并发编程/2.2.3_流式响应与实时通信.md)
* [2.2.4 AI-Agent的并发调度与协调](第二部分_Agentic_AI应用的开发基础/2.2_异步并发编程/2.2.4_AI-Agent的并发调度与协调.md)
* [2.2.5 异步数据处理与IO优化](第二部分_Agentic_AI应用的开发基础/2.2_异步并发编程/2.2.5_异步数据处理与IO优化.md)
* [2.2.6 微服务架构下的异步通信](第二部分_Agentic_AI应用的开发基础/2.2_异步并发编程/2.2.6_微服务架构下的异步通信.md)
* [2.2.7 性能监控与异步编程调优](第二部分_Agentic_AI应用的开发基础/2.2_异步并发编程/2.2.7_性能监控与异步编程调优.md)
* [2.2.8 异步编程的工程化实践](第二部分_Agentic_AI应用的开发基础/2.2_异步并发编程/2.2.8_异步编程的工程化实践.md)

### 2.3 状态机

* [2.3.1 Agentic AI应用中的状态机基础理论与设计原则](第二部分_Agentic_AI应用的开发基础/2.3_状态机/2.3.1_AI应用中的状态机基础理论与设计原则.md)
* [2.3.1.5 状态机设计方法论](第二部分_Agentic_AI应用的开发基础/2.3_状态机/2.3.1.5_状态机设计方法论.md)
* [2.3.2 Agent级状态机设计与实现](第二部分_Agentic_AI应用的开发基础/2.3_状态机/2.3.2_Agent级状态机设计与实现.md)
* [2.3.3 任务执行状态机与工作流管理](第二部分_Agentic_AI应用的开发基础/2.3_状态机/2.3.3_任务执行状态机与工作流管理.md)
* [2.3.4 工具调用状态机与异步执行管理](第二部分_Agentic_AI应用的开发基础/2.3_状态机/2.3.4_工具调用状态机与异步执行管理.md)
* [2.3.5 多Agent协作的分布式状态机](第二部分_Agentic_AI应用的开发基础/2.3_状态机/2.3.5_多Agent协作的分布式状态机.md)
* [2.3.6 前后端状态同步与实时通信状态机](第二部分_Agentic_AI应用的开发基础/2.3_状态机/2.3.6_前后端状态同步与实时通信状态机.md)
* [2.3.7 状态持久化与容错机制设计](第二部分_Agentic_AI应用的开发基础/2.3_状态机/2.3.7_状态持久化与容错机制设计.md)
* [2.3.8 性能优化与状态机监控](第二部分_Agentic_AI应用的开发基础/2.3_状态机/2.3.8_性能优化与状态机监控.md)
* [2.3.9 案例分析：企业级Agentic AI应用状态机架构实践](第二部分_Agentic_AI应用的开发基础/2.3_状态机/2.3.9_案例分析：企业级Agentic_AI应用状态机架构实践.md)
* [2.3.10 状态机设计的最佳实践与常见陷阱](第二部分_Agentic_AI应用的开发基础/2.3_状态机/2.3.10_状态机设计的最佳实践与常见陷阱.md)
* [代码示例库](第二部分_Agentic_AI应用的开发基础/2.3_状态机/代码示例库.md)

### 2.4 设计模式

* [2.4.1 创建型模式在Agentic AI应用中的应用](第二部分_Agentic_AI应用的开发基础/2.4_设计模式/2.4.1_创建型模式在AI应用中的应用.md)
* [2.4.2 结构型模式与Agentic AI组件设计](第二部分_Agentic_AI应用的开发基础/2.4_设计模式/2.4.2_结构型模式与AI组件设计.md)
* [2.4.3 行为型模式驱动的AI决策流程](第二部分_Agentic_AI应用的开发基础/2.4_设计模式/2.4.3_行为型模式驱动的AI决策流程.md)
* [2.4.4 责任链模式与智能体协作](第二部分_Agentic_AI应用的开发基础/2.4_设计模式/2.4.4_责任链模式与智能体协作.md)
* [2.4.5 依赖注入与配置管理模式](第二部分_Agentic_AI应用的开发基础/2.4_设计模式/2.4.5_依赖注入与配置管理模式.md)
* [2.4.6 设计模式最佳实践与反模式](第二部分_Agentic_AI应用的开发基础/2.4_设计模式/2.4.6_设计模式最佳实践与反模式.md)

## 第三部分：开发思想与设计哲学

### 3.1 面向黑盒编程

* [3.1.1 面向黑盒编程思想概述](第三部分_开发思想与设计哲学/3.1_面向黑盒编程/3.1.1_面向黑盒编程思想概述.md)
* [3.1.2 工具系统的黑盒化设计](第三部分_开发思想与设计哲学/3.1_面向黑盒编程/3.1.2_工具系统的黑盒化设计.md)
* [3.1.3 智能体的黑盒化架构](第三部分_开发思想与设计哲学/3.1_面向黑盒编程/3.1.3_智能体的黑盒化架构.md)
* [3.1.4 大语言模型的黑盒化调用](第三部分_开发思想与设计哲学/3.1_面向黑盒编程/3.1.4_大语言模型的黑盒化调用.md)
* [3.1.5 外部服务的黑盒化集成](第三部分_开发思想与设计哲学/3.1_面向黑盒编程/3.1.5_外部服务的黑盒化集成.md)
* [3.1.6 执行环境的黑盒化隔离](第三部分_开发思想与设计哲学/3.1_面向黑盒编程/3.1.6_执行环境的黑盒化隔离.md)
* [3.1.7 跨语言组件的黑盒化协作](第三部分_开发思想与设计哲学/3.1_面向黑盒编程/3.1.7_跨语言组件的黑盒化协作.md)
* [3.1.8 黑盒组件的监控与治理](第三部分_开发思想与设计哲学/3.1_面向黑盒编程/3.1.8_黑盒组件的监控与治理.md)

### 3.2 鲁棒性设计：AI系统的抗脆弱性工程学

* [3.2.0 鲁棒性设计 - 章节导览](第三部分_开发思想与设计哲学/3.2_鲁棒性设计/3.2.0_章节导引：鲁棒性设计.md)
* [3.2.1 大模型输出的不确定性处理](第三部分_开发思想与设计哲学/3.2_鲁棒性设计/3.2.1_大模型输出的不确定性处理.md)
* [3.2.2 任务执行的智能终止与验证](第三部分_开发思想与设计哲学/3.2_鲁棒性设计/3.2.2_任务执行的智能终止与验证.md)
* [3.2.3 多智能体协作中的容错与恢复](第三部分_开发思想与设计哲学/3.2_鲁棒性设计/3.2.3_多智能体协作中的容错与恢复.md)
* [3.2.4 自适应学习与动态调整机制](第三部分_开发思想与设计哲学/3.2_鲁棒性设计/3.2.4_自适应学习与动态调整机制.md)
* [3.2.5 工具生态的进化式适应](第三部分_开发思想与设计哲学/3.2_鲁棒性设计/3.2.5_工具生态的进化式适应.md)
* [3.2.6 不确定性驱动的系统架构设计](第三部分_开发思想与设计哲学/3.2_鲁棒性设计/3.2.6_不确定性驱动的系统架构设计.md)

### 3.3 合理分工：AI与传统技术的协同设计

* [3.3.0 合理分工 - 章节导览](第三部分_开发思想与设计哲学/3.3_合理分工/3.3.0_章节导引：合理分工.md)
* [3.3.1 认识大模型的能力边界与固有局限](第三部分_开发思想与设计哲学/3.3_合理分工/3.3.1_认识大模型的能力边界与固有局限.md)
* [3.3.2 构建合理的系统架构分层和职能分工](第三部分_开发思想与设计哲学/3.3_合理分工/3.3.2_构建合理的系统架构分层和职能分工.md)
* [3.3.3 充分发挥传统技术的优势](第三部分_开发思想与设计哲学/3.3_合理分工/3.3.3_充分发挥传统技术的优势.md)
* [3.3.4 构建企业级可靠性工程体系](第三部分_开发思想与设计哲学/3.3_合理分工/3.3.4_构建企业级可靠性工程体系.md)
* [3.3.5 性能优化与成本控制的工程实践](第三部分_开发思想与设计哲学/3.3_合理分工/3.3.5_性能优化与成本控制的工程实践.md)

## 第四部分：智能化工程

* [4.0 智能化工程简介](第四部分_智能化工程/4.0_智能化工程简介.md)

### 4.1 知识记忆层与上下文工程

* [4.1.0 知识记忆层概述](第四部分_智能化工程/4.1_知识记忆层与上下文工程/4.1.0_知识记忆层概述.md)
* [4.1.1 上下文工程的理论基础与技术架构](第四部分_智能化工程/4.1_知识记忆层与上下文工程/4.1.1_上下文工程的理论基础与技术架构.md)
* [4.1.2 AI记忆系统的分层架构设计](第四部分_智能化工程/4.1_知识记忆层与上下文工程/4.1.2_AI记忆系统的分层架构设计.md)
* [4.1.3 上下文管理的工程化实践](第四部分_智能化工程/4.1_知识记忆层与上下文工程/4.1.3_上下文管理的工程化实践.md)
* [4.1.4 检索增强生成(RAG)与上下文工程的深度融合](第四部分_智能化工程/4.1_知识记忆层与上下文工程/4.1.4_检索增强生成(RAG)与上下文工程的深度融合.md)
* [4.1.5 多Agent系统的上下文协同机制](第四部分_智能化工程/4.1_知识记忆层与上下文工程/4.1.5_多Agent系统的上下文协同机制.md)
* [4.1.6 上下文工程的性能优化与监控](第四部分_智能化工程/4.1_知识记忆层与上下文工程/4.1.6_上下文工程的性能优化与监控.md)
* [4.1.7 企业级上下文工程的安全与合规](第四部分_智能化工程/4.1_知识记忆层与上下文工程/4.1.7_企业级上下文工程的安全与合规.md)

### 4.2 智能思考工程

* [4.2.0 智能思考层概述](第四部分_智能化工程/4.2_智能思考工程/4.2.0_智能思考层概述.md)
* [4.2.1 智能思考工程基础](第四部分_智能化工程/4.2_智能思考工程/4.2.1_智能思考工程基础.md)
* [4.2.2 核心组件设计原理](第四部分_智能化工程/4.2_智能思考工程/4.2.2_核心组件设计原理.md)
* [4.2.3 主流思维模式设计](第四部分_智能化工程/4.2_智能思考工程/4.2.3_主流思维模式设计.md)
  * [4.2.3.1 ReAct思维模式](第四部分_智能化工程/4.2_智能思考工程/4.2.3.1_ReAct思维模式.md)
  * [4.2.3.2 Plan-Solve思维模式](第四部分_智能化工程/4.2_智能思考工程/4.2.3.2_Plan-Solve思维模式.md)
  * [4.2.3.3 多模式思维融合架构](第四部分_智能化工程/4.2_智能思考工程/4.2.3.3_多模式思维融合架构.md)
* [4.2.4 智能思考的工程化实践](第四部分_智能化工程/4.2_智能思考工程/4.2.4_智能思考的工程化实践.md)
* [4.2.5 高级思考工程技术](第四部分_智能化工程/4.2_智能思考工程/4.2.5_高级思考工程技术.md)
  * [4.2.5.1 元认知与反思机制](第四部分_智能化工程/4.2_智能思考工程/4.2.5.1_元认知与反思机制.md)
  * [4.2.5.2 分布式思考系统架构](第四部分_智能化工程/4.2_智能思考工程/4.2.5.2_分布式思考系统架构.md)
  * [4.2.5.3 思考过程的可观测性与调试](第四部分_智能化工程/4.2_智能思考工程/4.2.5.3_思考过程的可观测性与调试.md)

## 第五部分：核心进化

### 5.1 智能涌现

* [智能涌现层定义](第五部分_核心进化/5.1_智能涌现/智能涌现层定义.md)
* [5.1.1 智能涌现的理论基础](第五部分_核心进化/5.1_智能涌现/5.1.1_智能涌现的理论基础.md)
* [5.1.2 智能涌现层与其他核心层的关系分析](第五部分_核心进化/5.1_智能涌现/5.1.2_智能涌现层与其他核心层的关系分析.md)
* [5.1.3 多智能体协作机制](第五部分_核心进化/5.1_智能涌现/5.1.3_多智能体协作机制.md)
* [5.1.4 智能环境层架构设计](第五部分_核心进化/5.1_智能涌现/5.1.4_智能环境层架构设计.md)
* [5.1.5 工具系统的组合式演化](第五部分_核心进化/5.1_智能涌现/5.1.5_工具系统的组合式演化.md)
* [5.1.6 安全执行环境实现](第五部分_核心进化/5.1_智能涌现/5.1.6_安全执行环境实现.md)
* [5.1.7 涌现质量评估与优化](第五部分_核心进化/5.1_智能涌现/5.1.7_涌现质量评估与优化.md)
* [5.1.8 工程实践案例分析](第五部分_核心进化/5.1_智能涌现/5.1.8_工程实践案例分析.md)

### 5.2 智能态势感知

* [5.2.1 智能态势感知的理论基础](第五部分_核心进化/5.2_智能态势感知/5.2.1_智能态势感知的理论基础.md)
* [5.2.2 智能态势感知的技术架构](第五部分_核心进化/5.2_智能态势感知/5.2.2_智能态势感知的技术架构.md)
* [5.2.3 核心技术实现](第五部分_核心进化/5.2_智能态势感知/5.2.3_核心技术实现.md)
* [5.2.4 工程实践与系统集成](第五部分_核心进化/5.2_智能态势感知/5.2.4_工程实践与系统集成.md)
* [5.2.5 实战案例分析](第五部分_核心进化/5.2_智能态势感知/5.2.5_实战案例分析.md)
* [5.2.6 安全与隐私保护](第五部分_核心进化/5.2_智能态势感知/5.2.6_安全与隐私保护.md)
* [5.2.7 发展趋势与未来展望](第五部分_核心进化/5.2_智能态势感知/5.2.7_发展趋势与未来展望.md)

## 第六部分：参考资料

* [6.0 参考资料概述](第六部分_参考资料/6.0_参考资料概述.md)

### 6.1 JoyAgent & JDGenie 提示词库

* [提示词库概览](第六部分_参考资料/6.1_joyagent_jdgenie_提示词库/README.md)
* [提示词快速索引](第六部分_参考资料/6.1_joyagent_jdgenie_提示词库/提示词快速索引.md)
* [提示词统计报告](第六部分_参考资料/6.1_joyagent_jdgenie_提示词库/提示词统计报告.md)

#### 6.1.1 Agent代理相关

* [主代理系统提示词](第六部分_参考资料/6.1_joyagent_jdgenie_提示词库/6.1.1_Agent代理相关/主代理系统提示词.md)
* [任务规划代理提示词](第六部分_参考资料/6.1_joyagent_jdgenie_提示词库/6.1.1_Agent代理相关/任务规划代理提示词.md)
* [工具调用代理提示词](第六部分_参考资料/6.1_joyagent_jdgenie_提示词库/6.1.1_Agent代理相关/工具调用代理提示词.md)

#### 6.1.2 代码执行器相关

* [代码解释器提示词](第六部分_参考资料/6.1_joyagent_jdgenie_提示词库/6.1.2_代码执行器相关/代码解释器提示词.md)

#### 6.1.3 搜索分析相关

* [深度搜索提示词](第六部分_参考资料/6.1_joyagent_jdgenie_提示词库/6.1.3_搜索分析相关/深度搜索提示词.md)

#### 6.1.4 报告生成相关

* [报告生成提示词集合](第六部分_参考资料/6.1_joyagent_jdgenie_提示词库/6.1.4_报告生成相关/报告生成提示词集合.md)

### 6.2 OpenHands翻译版提示词

* [项目概览](第六部分_参考资料/6.2_OpenHands翻译版提示词/README.md)
* [快速导航](第六部分_参考资料/6.2_OpenHands翻译版提示词/快速导航.md)
* [整理报告](第六部分_参考资料/6.2_OpenHands翻译版提示词/整理报告.md)
* [更新日志](第六部分_参考资料/6.2_OpenHands翻译版提示词/更新日志.md)
* [项目总结](第六部分_参考资料/6.2_OpenHands翻译版提示词/项目总结.md)

#### 6.2.1 Agent核心提示词

* [CodeAct-Agent系统提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.1-Agent核心提示词/CodeAct-Agent系统提示词.md)
* [CodeAct-Agent交互规则](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.1-Agent核心提示词/CodeAct-Agent交互规则.md)
* [CodeAct-Agent任务管理](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.1-Agent核心提示词/CodeAct-Agent任务管理.md)
* [CodeAct-Agent技术哲学](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.1-Agent核心提示词/CodeAct-Agent技术哲学.md)
* [CodeAct-Agent示例对话](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.1-Agent核心提示词/CodeAct-Agent示例对话.md)
* [ReadOnly-Agent系统提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.1-Agent核心提示词/ReadOnly-Agent系统提示词.md)

#### 6.2.2 Resolver提示词

* [基础问题解决提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.2-Resolver提示词/基础问题解决提示词.md)
* [成功判断提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.2-Resolver提示词/成功判断提示词.md)

#### 6.2.3 集成提示词

* [GitHub集成提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.3-集成提示词/GitHub集成提示词.md)
* [GitLab集成提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.3-集成提示词/GitLab集成提示词.md)
* [Jira集成提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.3-集成提示词/Jira集成提示词.md)
* [其他集成提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.3-集成提示词/其他集成提示词.md)
* [总结提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.3-集成提示词/总结提示词.md)

#### 6.2.4 Microagents提示词

* [代码审查Microagents](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.4-Microagents提示词/代码审查Microagents.md)
* [开发工具Microagents](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.4-Microagents提示词/开发工具Microagents.md)
* [版本控制Microagents](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.4-Microagents提示词/版本控制Microagents.md)

#### 6.2.5 辅助提示词

* [可解决性评估提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.5-辅助提示词/可解决性评估提示词.md)
* [安全风险评估提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.5-辅助提示词/安全风险评估提示词.md)
* [记忆生成提示词](第六部分_参考资料/6.2_OpenHands翻译版提示词/6.2.5-辅助提示词/记忆生成提示词.md)

### 6.3 OpenHands原版提示词

* [项目概览](第六部分_参考资料/6.3_OpenHands原版提示词/README.md)
* [分类说明](第六部分_参考资料/6.3_OpenHands原版提示词/分类说明.md)
* [快速导航](第六部分_参考资料/6.3_OpenHands原版提示词/快速导航.md)
* [整理报告](第六部分_参考资料/6.3_OpenHands原版提示词/整理报告.md)
* [文件清单](第六部分_参考资料/6.3_OpenHands原版提示词/文件清单.txt)

#### 6.3.1 Agents

##### CodeAct-Agent
* [系统提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/CodeAct-Agent/system_prompt.j2)
* [交互式系统提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/CodeAct-Agent/system_prompt_interactive.j2)
* [长期任务系统提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/CodeAct-Agent/system_prompt_long_horizon.j2)
* [技术哲学提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/CodeAct-Agent/system_prompt_tech_philosophy.j2)
* [用户提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/CodeAct-Agent/user_prompt.j2)
* [上下文学习示例](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/CodeAct-Agent/in_context_learning_example.j2)
* [上下文学习示例后缀](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/CodeAct-Agent/in_context_learning_example_suffix.j2)
* [微代理信息](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/CodeAct-Agent/microagent_info.j2)
* [安全风险评估](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/CodeAct-Agent/security_risk_assessment.j2)
* [附加信息](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/CodeAct-Agent/additional_info.j2)

##### ReadOnly-Agent
* [系统提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/ReadOnly-Agent/system_prompt.j2)
* [用户提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/ReadOnly-Agent/user_prompt.j2)
* [上下文学习示例](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/ReadOnly-Agent/in_context_learning_example.j2)
* [上下文学习示例后缀](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/ReadOnly-Agent/in_context_learning_example_suffix.j2)
* [微代理信息](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/ReadOnly-Agent/microagent_info.j2)
* [附加信息](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.1-Agents/ReadOnly-Agent/additional_info.j2)

#### 6.3.2 集成提示词

##### GitHub集成
* [Issue对话指令](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/GitHub/issue_conversation_instructions.j2)
* [Issue提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/GitHub/issue_prompt.j2)
* [PR更新对话指令](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/GitHub/pr_update_conversation_instructions.j2)
* [PR更新提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/GitHub/pr_update_prompt.j2)

##### GitLab集成
* [Issue对话指令](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/GitLab/issue_conversation_instructions.j2)
* [Issue提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/GitLab/issue_prompt.j2)
* [MR更新对话指令](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/GitLab/mr_update_conversation_instructions.j2)
* [MR更新提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/GitLab/mr_update_prompt.j2)

##### Bitbucket集成
* [Issue对话指令](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Bitbucket/issue_conversation_instructions.j2)
* [Issue提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Bitbucket/issue_prompt.j2)
* [PR更新对话指令](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Bitbucket/pr_update_conversation_instructions.j2)
* [PR更新提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Bitbucket/pr_update_prompt.j2)

##### Jira集成
* [Jira指令](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Jira/jira_instructions.j2)
* [Jira新对话](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Jira/jira_new_conversation.j2)
* [Jira现有对话](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Jira/jira_existing_conversation.j2)

##### Jira-DC集成
* [Jira-DC指令](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Jira-DC/jira_dc_instructions.j2)
* [Jira-DC新对话](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Jira-DC/jira_dc_new_conversation.j2)
* [Jira-DC现有对话](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Jira-DC/jira_dc_existing_conversation.j2)

##### Linear集成
* [Linear指令](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Linear/linear_instructions.j2)
* [Linear新对话](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Linear/linear_new_conversation.j2)
* [Linear现有对话](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Linear/linear_existing_conversation.j2)

##### Slack集成
* [用户消息对话指令](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Slack/user_message_conversation_instructions.j2)

##### 建议任务
* [失败检查提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Suggested-Tasks/failing_checks_prompt.j2)
* [合并冲突提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Suggested-Tasks/merge_conflict_prompt.j2)
* [开放Issue提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Suggested-Tasks/open_issue_prompt.j2)
* [未解决评论提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/Suggested-Tasks/unresolved_comments_prompt.j2)

##### 其他集成
* [总结提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.2-Integrations/summary_prompt.j2)

#### 6.3.3 Resolver提示词

##### 成功判断
* [Issue成功检查](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.3-Resolver/Guess-Success/issue-success-check.jinja)
* [PR反馈检查](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.3-Resolver/Guess-Success/pr-feedback-check.jinja)
* [PR审查检查](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.3-Resolver/Guess-Success/pr-review-check.jinja)
* [PR线程检查](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.3-Resolver/Guess-Success/pr-thread-check.jinja)

#### 6.3.4 基准测试

##### NoCode-Bench
* [NoCode基准测试](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.4-Benchmarks/NoCode-Bench/nc.j2)

#### 6.3.5 Microagents

* [添加代理](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/add_agent.md)
* [添加仓库指令](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/add_repo_inst.md)
* [处理PR评论](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/address_pr_comments.md)
* [代理记忆](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/agent_memory.md)
* [Bitbucket微代理](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/bitbucket.md)
* [代码审查](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/code-review.md)
* [代码审查烘焙版](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/codereview-roasted.md)
* [默认工具](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/default-tools.md)
* [Docker微代理](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/docker.md)
* [修复测试](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/fix_test.md)
* [修复Python行过长](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/fix-py-line-too-long.md)
* [Flarglebargle](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/flarglebargle.md)
* [GitHub微代理](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/github.md)
* [GitLab微代理](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/gitlab.md)
* [Kubernetes微代理](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/kubernetes.md)
* [NPM微代理](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/npm.md)
* [入职指导](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/onboarding.md)
* [PDFLaTeX微代理](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/pdflatex.md)
* [Microagents概览](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/README.md)
* [安全微代理](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/security.md)
* [SSH微代理](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/ssh.md)
* [Swift Linux微代理](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/swift-linux.md)
* [更新PR描述](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/update_pr_description.md)
* [更新测试](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.5-Microagents/update_test.md)

#### 6.3.6 企业版

* [总结系统消息](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.6-Enterprise/summary_system_message.j2)
* [总结用户消息](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.6-Enterprise/summary_user_message.j2)

#### 6.3.7 其他

* [Dockerfile模板](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.7-Others/Dockerfile.j2)
* [生成记忆提示词](第六部分_参考资料/6.3_OpenHands原版提示词/6.3.7-Others/generate_remember_prompt.j2)
